...
"""
    ...
    def _is_codex_backend(config: Config) -> bool:
        from src.codex_adapter import is_codex_backend
        return is_codex_backend(config)

    @staticmethod
    def _codex_completion(call_kwargs: Dict[str, Any], config: Config) -> Any:
        from src.codex_adapter import codex_completion
        return codex_completion(call_kwargs, config)
        ...
        *,
        ...
        for model in models_to_try:
            try:
                ...
                if stream and not self._is_codex_backend(config):
                    try:
